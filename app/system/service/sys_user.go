...
func (s *sysUser) SendMail(req *model.SendMailReq) (err error)  {
	conn, err := grpc.Dial("docker-java:999", grpc.WithTransportCredentials(insecure.NewCredentials()))
	if err != nil {
		g.Log().Error(err)
		err = gerror.Newf("无法连接容器网络, %v", err)
		return
	}
	defer conn.Close()
